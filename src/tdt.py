...
class TemporalDifferenceTransformer(nn.Module):
    ...
    def forward(self, feat_t0: torch.Tensor, feat_t1: torch.Tensor):
        ...
        Returns:
            F_diff: [B, C, H', W'] temporal difference features
        """
        # Reshape spatial to sequence: [B, C, H, W] -> [B, N, C]
        B, C, H, W = feat_t0.shape
        feat_t0 = feat_t0.flatten(2).transpose(1, 2)  # [B, H*W, C]
        feat_t1 = feat_t1.flatten(2).transpose(1, 2)
        
        # Add temporal embeddings
        N = feat_t0.shape[1]
        feat_t0 = feat_t0 + self.temporal_embed_t0.expand(B, N, -1)
        feat_t1 = feat_t1 + self.temporal_embed_t1.expand(B, N, -1)
        ...
        for block in self.blocks:
            feat_t0, feat_t1 = block(feat_t0, feat_t1)
        
        # Compute difference: what changed
        diff_forward = feat_t0 - orig_t0  # What appeared/changed (t0→t1)
        diff_backward = feat_t1 - orig_t1  # What disappeared/changed (t1→t0)
